...
public class GymDAOImplementation implements GymDAOInterface {
            ...
            if (rowsInserted > 0) {
                ...
                try (ResultSet resultSet = preparedStatement.getGeneratedKeys()) {
                    if (resultSet.next()) {
                        int gymId = resultSet.getInt(1);
                        gym.setGymId(gymId);
                        insertSlots(gym.getSlots(), gymId, conn);
                    }
                }
            } else {
                throw new SQLException("Failed to add the gym.");
    ...
    private void insertSlots(List<Slots> slots, int gymId, Connection conn) throws SQLException {
        String insertSlotQuery = "INSERT INTO Slots (start_time, seat_count, gym_id) VALUES (?, ?, ?)";

        try (PreparedStatement preparedStatement = conn.prepareStatement(insertSlotQuery)) {
            for (Slots slot : slots) {
                preparedStatement.setString(1, slot.getTimeSlot());
                preparedStatement.setInt(2, slot.getSeatCount());
                preparedStatement.setInt(3, gymId);
                preparedStatement.addBatch();
            }
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    List<Slots> slotsList = getSlotsByGymId(gym.getGymId(), conn);
                    gym.setSlots(slotsList);
        ...
        try (PreparedStatement preparedStatement = conn.prepareStatement(query)) {
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    int seatCount = resultSet.getInt("seat_count");
                    Slots slot = new Slots(slotsId, timeSlot, seatCount);
                    slotsList.add(slot);
                }
            }
        } catch (SQLException e) {
